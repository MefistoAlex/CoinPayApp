...
public protocol ICountriesPickerUseCase: Sendable {
...
public final class CountriesPickerUseCase: ICountriesPickerUseCase, Sendable {
...
//    private lazy var allCountries = countriesRepository.getAllCountries()
    ...
    public init(countriesRepository: ICountriesRepository) {
    ...
    public func searchSections(search: String = "") async -> [CountrySection] {
        let allCountries = countriesRepository.getAllCountries()
        let countryCode = await locationManager.getCountryCode()
